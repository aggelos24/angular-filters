...
export class AppComponent implements OnInit, OnDestroy {
	...
	generateSuggestions(inputName: string) {
		...
		if (this.input[this.selectedInput].length < 3) {					//if input's length is less than 3 characters
		...
		for (let row of this.savedData) {
			...
			if (existsInSuggestions == false) {								//if there is no duplicate
				...
				tempArray = row[this.selectedInput].split(regex);			//split string by regular expression and store the result in array
